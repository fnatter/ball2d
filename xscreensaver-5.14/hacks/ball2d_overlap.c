...
Ball_doCollision(Ball* b1, Ball* b2)
{
  ...
  if (separationWithInverseVelocitiesSucceeded)
  ...
  else
    {
      ...
      /* direction b1 -> b2 */
      b->x = position_b1.x; b->y = position_b1.y;
      b2->x = position_b2.x; b2->y = position_b2.y;
...
static void *
ball2d_overlap_init (Display *dpy, Window window)
{
...
ball2d_overlap_draw (Display *dpy, Window window, void *closure)
{
  int i;
...
static void
ball2d_overlap_reshape (Display *dpy, Window window, void *closure, 
...
static Bool
ball2d_overlap_event (Display *dpy, Window window, void *closure, XEvent *event)
{
  return False;
}

static void
ball2d_overlap_free (Display *dpy, Window window, void *closure)
{
  struct state *st = (struct state *) closure;
  int i;
...
static const char *ball2d_overlap_defaults [] = {
  ".background:		black",
  ".foreground:		white",
...
static XrmOptionDescRec ball2d_overlap_options [] = {
...
XSCREENSAVER_MODULE ("Ball2D Overlap", ball2d_overlap)

/*
  Local Variables:
  compile-command: "make ball2d_overlap"
  End:
*/
