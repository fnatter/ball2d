...
static number
Ball_findWallIntersection(Ball* b)
{
  number t[4];

  /* future collision with left wall?
     (b->x + b->dx*b->velocity*t = MINX + b->radius) */
  t[0] = (MINX - b->x + b->radius)/(b->dx * b->velocity);
  if (t[0] < 0)
    t[0] = INFINITY;

  /* future collision with right wall?
     (b->x + b->dx*b->velocity*t = MAXX - b->radius) */
  t[1] = (MAXX - b->x - b->radius)/(b->dx * b->velocity);
  if (t[1] < 0)
    t[1] = INFINITY;

  /* future collision with top wall?
     (b->y + b->dy*b->velocity*t = MINY + b->radius) */
  t[2] = (MINY - b->y + b->radius)/(b->dy * b->velocity);
  if (t[2] < 0)
    t[2] = INFINITY;
  
  /* future collision with bottom wall?
     (b->y + b->dy*b->velocity*t = MAXY - b->radius) */
  t[3] = (MAXY - b->y - b->radius)/(b->dy * b->velocity);
  if (t[3] < 0)
    t[3] = INFINITY;

  return fmin(fmin(fmin(t[0], t[1]), t[2]), t[3]);
}
...
Ball_findObstacleIntersection(Ball* b, Obstacle* ob)
{
  ...
  if (b->x < center.x && b->y < center.y)
    t[4] = Ball_findPathPointIntersection(b, ob->x1, ob->y1);
  else
    t[4] = INFINITY;

  /* future intersection with top right corner? */
  if (b->x > center.x && b->y < center.y)
    t[5] = Ball_findPathPointIntersection(b, ob->x2, ob->y1);
  else
    t[5] = INFINITY;

  /* future intersection with bottom left corner? */
  if (b->x < center.x && b->y > center.y)
    t[6] = Ball_findPathPointIntersection(b, ob->x1, ob->y2);
  else
    t[6] = INFINITY;

  /* future intersection with bottom right corner? */
  if (b->x > center.x && b->y > center.y)
    t[7] = Ball_findPathPointIntersection(b, ob->x2, ob->y2);
  else
    t[7] = INFINITY;

  return fmin(fmin(fmin(fmin(fmin(fmin(fmin(t[0], t[1]), t[2]), t[3]), t[4]), t[5]), t[6]), t[7]);
}
...
static Bool
Ball_future_collision_check(Ball* b1, number stepX, number stepY,
                            Ball* b2)
{
  double r1pr2, dx, dy;
  r1pr2 = b1->radius + b2->radius;
...
Ball_draw (struct state* st, Drawable w, Ball *ball)
{
  ...
  if (st->nextBallCollision.b1 == ball ||
      st->nextBallCollision.b2 == ball)
    {
      ...
      coll.x = ball->x +
        ball->dx * ball->velocity * st->nextBallCollision.t;
      coll.y = ball->y +
        ball->dy * ball->velocity * st->nextBallCollision.t;
      /* get the center of b2 at collision time */
      collOther.x = otherBall->x +
        otherBall->dx * otherBall->velocity * st->nextBallCollision.t;
      collOther.y = otherBall->y +
        otherBall->dy * otherBall->velocity * st->nextBallCollision.t;
      ...
      for (i = 0; i < nBalls; i++)
        {
          ...
          stepX = cball->dx * thisway;
          stepY = cball->dy * thisway;
#if 0
          printf("Balls[%d] = (%.1f,%.1f)\n", i, cball->x, cball->y);
#endif
          if (cball->x + stepX + cball->radius > MAXX)
            {
          ...
          if (cball->y + stepY + cball->radius > MAXY)
            {
          ...
          if (cball->x + stepX < MINX + cball->radius)
            {
          ...
          if (cball->y + stepY < MINY + cball->radius)
            {
          ...
          for (j = 0; j < i; j++)
            {
              if (Ball_future_collision_check(cball, stepX, stepY, 
                                              balls + j))
                {
          ...
          for (j = 0; j < i; j++)
            {
              ...
              if (t < st->nextBallCollision.t)
                {
                  /* is there a wall collision before this ball collision? */
                  /* TODO: should cache these findWallIntersection calls! */
                  if (Ball_findWallIntersection(cball) > t &&
                      Ball_findWallIntersection(&st->balls[j]) > t)
                    {
                      Bool obstacleIntersectionBeforeBallIntersection = False;
                      int k;

                      /* TODO: should cache these findObstacleIntersection calls! */
                      for (k = 0; k < st->nObstacles; k++)
                        {
                          if (Ball_findObstacleIntersection(cball, &st->obstacles[k]) < t ||
                              Ball_findObstacleIntersection(&st->balls[j], &st->obstacles[k]) < t)
                            obstacleIntersectionBeforeBallIntersection = True;
                        }

                      if (!obstacleIntersectionBeforeBallIntersection)
                        {
...
static void *
ball2d_lookahead_init (Display *dpy, Window window)
{
...
ball2d_lookahead_draw (Display *dpy, Window window, void *closure)
{
  int i;
...
static void
ball2d_lookahead_reshape (Display *dpy, Window window, void *closure, 
...
static Bool
ball2d_lookahead_event (Display *dpy, Window window, void *closure, XEvent *event)
{
  return False;
}

static void
ball2d_lookahead_free (Display *dpy, Window window, void *closure)
{
  struct state *st = (struct state *) closure;
  int i;
...
static const char *ball2d_lookahead_defaults [] = {
  ".background:		black",
  ".foreground:		white",
...
static XrmOptionDescRec ball2d_lookahead_options [] = {
...
XSCREENSAVER_MODULE ("Ball2D_Lookahead", ball2d_lookahead)

/*
  Local Variables:
  compile-command: "make ball2d_lookahead"
  End:
*/
