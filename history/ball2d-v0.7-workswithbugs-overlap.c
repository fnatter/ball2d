...
static void
Ball_draw (struct state* st, Drawable w, Ball *ball, Bool highlight)
{
  ...
  if (highlight || st->highlightedBall == ball)
    /*
    XFillRectangle(st->dpy, w, st->highlightGC,
                   ...
                   radiusX*2, radiusY*2);
    */             
    XFillArc (st->dpy, w, st->highlightGC,
              deviceX - radiusX,
              deviceY - radiusY,
              radiusX*2, radiusY*2,
              0, 360*64);
          ...
          for (j = 0; j < st->nObstacles; j++)
            {
              if (Ball_Obstacle_collision_check(cball, &st->obstacles[j], &obsCollType))
                Ball_doObstacleCollision(st, cball, &st->obstacles[j]);
            }
...
ball2d_draw (Display *dpy, Window window, void *closure)
{
  ...
  for (i = 0; i < st->count; i++)
    {
      Ball_draw(st, st->b, &st->balls[i], False);
    }
