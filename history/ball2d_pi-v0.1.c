...
Ball_draw (struct state* st, Drawable w, Ball *ball)
{
  ...
  XFillArc (st->dpy, w, ball->gc,
            ...
            radiusX*2, radiusY*2,
            0, 360*64);
  if (st->nextEvent.type == BALL_COLLISION &&
...
Balls_move(struct state* st)
{
  ...
  if (st->nextEvent.delta_t < SINGLE_TIME_STEP)
    {
    ...
    {
      this_delta_t = SINGLE_TIME_STEP;
      st->nextEvent.delta_t -= this_delta_t;
    }

#ifdef EVENT_LOGGING
  printf("this_delta_t=%.2f\n", this_delta_t);
#endif
...
static void *
ball2d_pi_init (Display *dpy, Window window)
{
...
ball2d_pi_draw (Display *dpy, Window window, void *closure)
{
  int i;
...
static void
ball2d_pi_reshape (Display *dpy, Window window, void *closure, 
...
static Bool
ball2d_pi_event (Display *dpy, Window window, void *closure, XEvent *event)
{
  return False;
}

static void
ball2d_pi_free (Display *dpy, Window window, void *closure)
{
  struct state *st = (struct state *) closure;
  int i;
...
static const char *ball2d_pi_defaults [] = {
  ".background:		black",
  ".foreground:		white",
...
static XrmOptionDescRec ball2d_pi_options [] = {
...
XSCREENSAVER_MODULE ("Ball2D PathIntersection", ball2d_pi)

/*
  Local Variables:
  compile-command: "make ball2d_pi"
  End:
*/
